...
MODPROBE_OUTPUT=$(shell lsmod | grep hddled_tmj33)
...
HDDLED_TMJ33_CFLAGS=-DHDDLED_TMJ33_VERSION='\"$(DRIVER_VERSION)\"'

modules:
	@$(MAKE) EXTRA_CFLAGS="$(HDDLED_TMJ33_CFLAGS)" -C $(KERNEL_BUILD) M=$(CURDIR) $@
...
dkms:
	...
	@cp `pwd`/hddled_tmj33.c $(DKMS_ROOT_PATH)
